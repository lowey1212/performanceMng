...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "PerfMngr|Proxy",
              meta = (ToolTip = "Swap to the proxy (cheap) version of this actor when safe to do so."))

    void SwapToProxy();

    /** Swap back to the full (high‑quality) representation when safe to do so. */
    UFUNCTION(BlueprintCallable, Category = "PerfMngr|Proxy",
              meta = (ToolTip = "Swap back to the full (high‑quality) version of this actor when safe to do so."))
    void SwapToFull();
    ...
    /** Lightweight access to the last computed significance value (0..1, higher = more significant). */
    UFUNCTION(BlueprintPure, Category = "PerfMngr",
              meta = (ToolTip = "Returns the last significance value computed for this actor (0..1, higher = more important)."))
    float GetCurrentSignificance() const { return LastSignificance; }
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite,
              ...
              meta = (ToolTip = "Billboard mesh used when significance is very low. Can be batched via HISM for large crowds. Ignored if ProxyBillboardEffect is set."))
    UStaticMesh *ProxyBillboardMesh = nullptr;

    /** Niagara effect used as a sprite-based billboard when Niagara is available. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite,
              Category = "PerfMngr|Proxy|Billboard",
              meta = (ToolTip = "Sprite Niagara effect to use instead of the static billboard mesh. Faces the camera automatically and is cheaper when Niagara is present."))
    ...
    UPROPERTY(Transient)
    UStaticMeshComponent *ProxyMeshComponent = nullptr;
    UPROPERTY(Transient)
    UStaticMeshComponent *ProxyHairMeshComponent = nullptr;
    UPROPERTY(Transient)
    UStaticMeshComponent *BillboardMeshComponent = nullptr;
    UPROPERTY(Transient)
    UNiagaraComponent *ProxyEffectComponent = nullptr;
    UPROPERTY(Transient)
    UNiagaraComponent *BillboardEffectComponent = nullptr;
