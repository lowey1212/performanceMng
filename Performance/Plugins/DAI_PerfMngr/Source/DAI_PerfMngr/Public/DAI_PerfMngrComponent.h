 ...
 *
 * In plain English: this speeds things up by showing cheap stand‑ins (proxies)
 * for far‑away actors and the full, expensive version when the actor is close
 * enough to matter.
 */
UENUM(BlueprintType)
...
/**
 * @brief Types of components this system can temporarily turn off (suppress)
 * when the actor is unimportant (low significance).
 *
 ...
 * drops.
 *
 * Layman’s terms: When the actor is far away (not important), these rules tell
 * the system which bits to switch off and when.
 */
...
struct FComponentSuppressionRule {
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Suppression",
            meta = (ToolTip = "What type of component this rule affects (e.g., "
                              "Audio, Niagara, Hair)."))
  ESuppressionComponentType ComponentType = ESuppressionComponentType::Audio;

  /** Only affect components with this tag. Leave blank to affect all of the
   * chosen type. */
  UPROPERTY(
      EditAnywhere, BlueprintReadWrite, Category = "Suppression",
      meta =
          (ToolTip = "Optional filter: only components with this tag are "
                     "affected. Leave empty to match all of the chosen type."))
  FName ComponentTag;

  /** Only affect components whose name contains this text (case‑insensitive).
   */
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Suppression",
            meta = (ToolTip = "Optional filter by name: only components with "
                              "names containing this text are affected."))
  FString NameContains;

  /** If the actor’s significance drops below this value, suppress this
   * component. */
  UPROPERTY(
      EditAnywhere, BlueprintReadWrite, Category = "Suppression",
      meta = (ClampMin = "0.0", ClampMax = "1.0",
              ToolTip =
                  "When the actor’s significance is below this value (0..1), "
                  "this component gets turned off to save performance."))
 ...
 * auto‑optimize.
 *
 * It monitors the actor’s distance/importance and swaps meshes/effects to
 * cheaper versions or reduces tick rates to save frame time.
 */
...
public:
  ...
  UFUNCTION(BlueprintCallable, Category = "PerfMngr|Proxy",
            meta = (ToolTip = "Gets the current proxy swap state (Active, "
                              "ProxyActive, or pending states)."))
