...
void UDAI_ProxyHISMManager::QueueInstanceForBatch(FName Tag, const FTransform &Transform)
{
    ...
    Queue.RemoveAll([&Transform](const FTransform &Existing)
...
UHierarchicalInstancedStaticMeshComponent *UDAI_ProxyHISMManager::GetOrCreateHISMForTag(FName Tag, UStaticMesh *Mesh, UObject *Outer)
{
    ...
    if (!Mesh)
    {
        UE_LOG(LogDAIPerfMngr, Error, TEXT("GetOrCreateHISMForTag: Mesh is null!"));
        return nullptr;
    }
    if (!Outer)
    {
        UE_LOG(LogDAIPerfMngr, Error, TEXT("GetOrCreateHISMForTag: Outer is null!"));
        return nullptr;
    }
    ...
    {
        if (GEngine)
            GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("No World found for HISM!"));
        return nullptr;
    }
    ...
    {
        if (GEngine)
            GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("No ProxyHISMRootActor found/spawned in world!"));
        return nullptr;
    }
...
void UDAI_ProxyHISMManager::RemoveInstanceByIndex(FName Tag, int32 Index)
{
    ...
    if (UHierarchicalInstancedStaticMeshComponent *HISM = TagToHISM.FindRef(Tag))
    {
        if (Index >= 0 && Index < HISM->GetInstanceCount())
        {
            ...
            if (TArray<FTransform> *Queue = PendingBatchedAdds.Find(Tag))
            {
                Queue->RemoveAll([&InstTransform](const FTransform &Existing)
