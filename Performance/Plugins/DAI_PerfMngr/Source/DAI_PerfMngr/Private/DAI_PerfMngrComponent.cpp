...
#include "DAIPerfMngrBusPayloads.h"
#include "DAI_PerfMngrLog.h"

// Optional message bridge (soft dependency)
...
namespace DAIMessageBridge
{
  template <typename T>
  inline void Broadcast(UObject *, const FGameplayTag &, const T &) {}
}
#endif
...
void UDAI_PerfMngrComponent::SwapToProxy()
{
  ...
  if (!Owner)
  {
    UE_LOG(LogDAIPerfMngr, Error, TEXT("[PerfMngr] SwapToProxy: Owner is null!"));
    return;
  }

  if (!ProxyStaticMesh)
  {
    UE_LOG(LogDAIPerfMngr, Error,
  ...
  if (ProxyStaticMesh && Owner)
  {
    ...
    if (World)
    {
              ...
              World->GetSubsystem<UDAI_ProxyHISMManager>())
      {
        ...
        UE_LOG(LogDAIPerfMngr, Warning, TEXT("SwapToProxy: Owner: %s Mesh: %s"),
      ...
      {
        UE_LOG(LogDAIPerfMngr, Error, TEXT("SwapToProxy: ProxyMgr is null!"));
      }
    }
  }
                                             ...
                                             float Significance)
{
  ...
  case EProxySwapState::PendingSwapToProxy:
        ...
        ProxyTimeInCurrentState >= ProxySwapDelay)
    {
      ...
      // Forward to internal message bus as a proxy-swap event
#if DAI_ENABLE_GAMEPLAY_MESSAGE_BUS && DAI_HAS_MESSAGE_BRIDGE
      {
        static FGameplayTag EnteredTag = FGameplayTag::RequestGameplayTag(FName("DAI.Perf.Proxy.Entered"));
        FDAIMessage_PerfProxySwap Payload;
        Payload.Component = this;
        Payload.State = EnteredTag;
        DAIMessageBridge::Broadcast(this, EnteredTag, Payload);
      }
#endif
    }
  ...
  case EProxySwapState::PendingSwapToFull:
        ...
        ProxyTimeInCurrentState >= ProxySwapDelay)
    {
      ...
      // Forward to internal message bus as a proxy-swap exit
#if DAI_ENABLE_GAMEPLAY_MESSAGE_BUS && DAI_HAS_MESSAGE_BRIDGE
      {
        static FGameplayTag ExitedTag = FGameplayTag::RequestGameplayTag(FName("DAI.Perf.Proxy.Exited"));
        FDAIMessage_PerfProxySwap Payload;
        Payload.Component = this;
        Payload.State = ExitedTag;
        DAIMessageBridge::Broadcast(this, ExitedTag, Payload);
      }
#endif
    }
...
void UDAI_PerfMngrComponent::UpdateTickBasedOnSignificance()
{
  ...
  LastSignificance = Significance;

  if (bEnableDebugLog)
  {
    UE_LOG(LogDAIPerfMngr, Log, TEXT("[PerfMngr] Significance: %.2f"), Significance);
  }
                                                      ...
                                                      float Significance)
{
  ...
  case EProxySwapState::PendingSwapToProxy:
  {
    ...
    {
      // Cache subsystem reference to avoid repeated lookups
      UDAI_ProxyHISMManager *ProxyMgr = GetWorld()->GetSubsystem<UDAI_ProxyHISMManager>();

      // Remove the existing proxy HISM instance
      if (bHasHISMInstance && ProxyStaticMesh && ProxyMgr)
      {
        ProxyMgr->RemoveInstanceAtTransform(ProxyHISMTag, ProxyHISMTransform);
        bHasHISMInstance = false;
      }
      ...
      else if (ProxyBillboardMesh && ProxyMgr)
      {
  ...
  case EProxySwapState::PendingSwapToFull:
  {
    ...
    if (BillboardTimeInCurrentState >= ProxySwapDelay)
    {
      ...
      else if (bHasHISMInstance && ProxyBillboardMesh && ProxyMgr)
      {
        ProxyMgr->RemoveInstanceAtTransform(ProxyHISMTag, ProxyHISMTransform);
        bHasHISMInstance = false;
      }
      // Restore the proxy instance via HISM
      if (ProxyStaticMesh && ProxyMgr)
      {
