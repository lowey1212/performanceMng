...
{
    // Multicast runs on server and clients; the server already applied the change
    // before broadcasting, so only apply on non-authority to avoid duplicate work.
    if (GetOwnerRole() != ROLE_Authority)
    {
        InternalSetComponentActive(Target, Type, bActive);
    }
...
#if WITH_EDITOR
        UE_LOG(LogDAIPerfMngr, Verbose, TEXT("Set component %s active %s"), *GetNameSafe(Comp), bActive ? TEXT("true") : TEXT("false"));
#endif
    }
}
